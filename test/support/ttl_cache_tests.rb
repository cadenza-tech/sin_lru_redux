...
require_relative 'cache_tests'

module TtlCacheTests
  include CacheTests

  def setup
    Timecop.freeze(Time.now)
    @cache = cache_class.new(3, 5 * 60, false)
  end
  ...
  def test_validate_max_size!
    assert_raises(ArgumentError) do
      cache_class.new('invalid', 0, false)
    end
    assert_raises(ArgumentError) do
      cache_class.new(0, 0, false)
    end
  end

  def test_validate_ttl!
    assert_raises(ArgumentError) do
      cache_class.new(1, 'invalid', false)
    end
    assert_raises(ArgumentError) do
      cache_class.new(0, -1, false)
    end
  end

  def test_validate_ignore_nil!
    assert_raises(ArgumentError) do
      cache_class.new(1, 0, 'invalid')
    end
  end
