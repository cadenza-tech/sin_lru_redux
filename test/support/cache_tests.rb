...
# Including classes must define a private #cache_class returning the class under test.
module CacheTests
  def setup
    @cache = cache_class.new(3, false)
  end
  ...
  def test_ttl_set
    ...
    assert_nil(@cache.send(:ttl=, 5 * 60))
  ...
  def test_each
    ...
    [:each, :each_unsafe].each do |method_name|
      keys = []
      values = []
      @cache.public_send(method_name) do |key, value|
        keys << key
        values << value
      end
  ...
  def test_key?
    ...
    assert(@cache.key?(:a))
    refute(@cache.key?(:b))
    assert(@cache.has_key?(:a)) # rubocop:disable Style/PreferredHashMethods
    refute(@cache.has_key?(:b)) # rubocop:disable Style/PreferredHashMethods
  end
  ...
  def test_validate_max_size!
    assert_raises(ArgumentError) do
      cache_class.new('invalid', false)
    end
    assert_raises(ArgumentError) do
      cache_class.new(0, false)
    end
  end

  def test_validate_ignore_nil!
    assert_raises(ArgumentError) do
      cache_class.new(1, 'invalid')
    end
  end
